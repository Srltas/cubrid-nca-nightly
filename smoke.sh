...
step "manager server is up and answering"
docker exec "$NAME" gosu cubrid cubrid manager status 2>&1 | grep -q "is running" \
  || fail "manager server is not running"
# The port has to be reachable from outside, which is how NCA reaches it.
docker exec "$NAME" bash -c 'exec 3<>/dev/tcp/127.0.0.1/8001' \
  || fail "manager port 8001 is not accepting connections"
...
docker exec "$NAME" gosu cubrid cubrid manager status 2>&1 | grep -q "is running" \
  || fail "manager server did not come back after the restart"
